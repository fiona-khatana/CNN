...
class fc(object):
	...
	def forward(self, feat):
		...
		#############################################################################
		x = feat.reshape(feat.shape[0], np.prod(feat.shape[1:]))
	...
	def backward(self, dprev):
		...
		W = np.transpose(self.params[self.w_name])
		X = self.meta
		dfeat = np.matmul(dprev, W).reshape(X.shape)

		self.grads[self.w_name] =np.matmul(np.transpose(X.reshape(X.shape[0], np.prod(X.shape[1:]))), dprev)
...
class cross_entropy(object):
	...
	def forward(self, feat, label):
		...
		#############################################################################
		self.N = feat.shape[0]
		loss = -np.sum(np.log(scores[np.arange(self.N), label])) / self.N
		#############################################################################
		#                             END OF YOUR CODE                              #
		#############################################################################
	...
	def backward(self):
		...
		dLoss[np.arange(self.N), self.label] -= 1
		dLoss = dLoss/ self.N
		#############################################################################
		#                             END OF YOUR CODE                              #
		#############################################################################
